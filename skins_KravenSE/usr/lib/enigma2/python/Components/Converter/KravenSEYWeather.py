...
class KravenSEYWeather(Poll, Converter, object):
	direction = 1
	speed = 2
	humidity = 3
	wtext = 4
	...
	def write_none(self):
		self.iConsole.ePopen("echo -e 'None' >> /tmp/KravenSEweather.xml")
		
	def get_xmlfile(self):
		self.iConsole.ePopen("wget -P /tmp -T2 'http://xml.weather.yahoo.com/forecastrss?w=%s&u=c' -O /tmp/KravenSEweather.xml" % config.plugins.KravenSE.weather_city.value, self.control_xml)
	...
	def getText(self):
		fweather, tweather = [], []
		xweather = {'feels':"N/A", 'ydirection':"N/A", 'yspeed':"N/A", 'yhumidity':"N/A", 'ytext':"N/A", 'ytemp':"N/A", 'ypicon':"3200", 'ymetric':"N/A"}
		direct = 0
		info = ''
		if fileExists("/tmp/KravenSEweather.xml"):
			if int((time.time() - os.stat("/tmp/KravenSEweather.xml").st_mtime)/60) >= time_update:
				self.get_xmlfile()
		else:
			self.get_xmlfile()
			if not fileExists("/tmp/KravenSEweather.xml"):
				self.write_none()
				return 'N/A'
                if not fileExists("/tmp/KravenSEweather.xml"):
			self.write_none()
			return 'N/A'
		for line in open("/tmp/KravenSEweather.xml"):
			if "<yweather:units" in line:
...
#####################################################
			...
			info = config.plugins.KravenSE.weather_city.value
                return info
######################################################
